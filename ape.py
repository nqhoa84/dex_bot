...
# ======================START RUNNING LOOP TO CHECK PRICE====================
    while (not exit_condition) and (not ProgramTerminated):
        ...
        try:
            ...
            elif (trend == Trend.DropTo) and (current_price <= value):
            ...
            elif trend == Trend.ChangeIsOver:
                PushPrice(price_q, current_price)
                if DataValid(price_q):
                    #diff2 = Diff2(price_q) 
                    if PricePumpOver(price_q, value):
                        logging.info("%s current_price: %9f Change is %f, threshold: %f", pair_name, current_price, Diff2(price_q), value)
                        logging.info("Block %i, %s do_a_swap at %9f", latestBlock, pair_name, current_price) 
                            
                        if action == Auto.Sell:
                            Sell(token, trans_data, current_price, decimal_list)
                        elif action == Auto.Buy:
                            logging.error("Should not buy when pumping")
                            #Buy(token, trans_data, current_price, decimal_list)

                        exit_condition = (freq == Freq.OneTime) 
                            
                        ClearPriceQ(price_q)
                        #this function take ~6s to complete, so it should be exec at last. 
                        Notify(pair_name + " increases suddenly " + str(Diff2(price_q) * 100) + "%") 
                         
                    elif PriceDumpOver(price_q, value):
                        logging.info("%s current_price: %9f Change is %f, threshold: %f", pair_name, current_price, Diff2(price_q), value)
                        logging.info("Block %i, %s do_a_swap at %9f", latestBlock, pair_name, current_price) 
                        
                        if action == Auto.Sell:
                            logging.error("should not sell when dropping")
                            #Sell(token, trans_data, current_price, decimal_list)
                        elif action == Auto.Buy:
                            Buy(token, trans_data, current_price, decimal_list)
                        ...
                        ClearPriceQ(price_q)
                        Notify(pair_name + " descreases suddenly " + str(Diff2(price_q) * 100) + "%")
